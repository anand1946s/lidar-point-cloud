...
def main():
    ...
    # -------------------------------------------------------------
    # NEW MODIFICATION: Create an output folder and save the file
    # -------------------------------------------------------------
    output_dir = "output"
    ...
    output_path = os.path.join(output_dir, "sift_keypoints.ply")
    
    print(f"Saving keypoint cloud to {output_path}...")
    # Writes the point cloud data directly out as a .ply file
    success = o3d.io.write_point_cloud(output_path, keypoints)
    if success:
        print("Keypoints successfully saved!")
    else:
        print("Failed to save the keypoints file.")
    # -------------------------------------------------------------

    # 3. Create visual marker spheres for keypoints
    visual_keypoints = o3d.geometry.TriangleMesh()
    # MODIFICATION: Reduce the diameter of the red dots
    for pt in keypoints.points:
        # Create smaller spheres
        sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.00075) 
        sphere.translate(pt)
    ...
    # MODIFICATION: Do not include the base point cloud (pcd) in the visualization
    o3d.visualization.draw_geometries([visual_keypoints], 
                                      window_name="Custom 3D-SIFT on bun090.ply - Keypoints Only")
